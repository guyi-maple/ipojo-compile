...
public class MavenHelper {
    ...
    private static Map<String,Set<tech.guyi.ipojo.compile.lib.configuration.entry.Dependency>> dependencyCache = new HashMap<>();
        ...
        if (Files.exists(Paths.get(dependencyCachePath))){
            try {
                dependencyCache = gson.fromJson(
                        ...
                        new TypeToken<Map<String,Set<tech.guyi.ipojo.compile.lib.configuration.entry.Dependency>>>(){}.getType()
                );
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    @SneakyThrows
    private static void addCache(String name, Set<tech.guyi.ipojo.compile.lib.configuration.entry.Dependency> dependencies){
    ...
    public static Set<tech.guyi.ipojo.compile.lib.configuration.entry.Dependency> getDependencies(
            Project project,
        ...
        try {
            ...
            Set<tech.guyi.ipojo.compile.lib.configuration.entry.Dependency> dependencies = new HashSet<>();
